...
// loop through each book title and author
(async function () {
  for (let i = 38; i < books.length; i++) {
    // const [title, author] = books[i];

    const title = books[i].book_name
    const author = books[i].author
    // console.log(title, author)

    // construct the query to search for the ISBN
    const query = `${title} ${author} ISBN`;
    // console.log(query)
    // console.log(`${encodeURIComponent(query)}`)

    try {
      // send a request to a search engine to retrieve the HTML
      ...
      // extract the ISBN from the HTML using Cheerio
      const $ = cheerio.load(html);
      const isbn = $('span:contains("ISBN")').first().text().match(/\d+/)[0];

      console.log(`${title} by ${author} has ISBN ${isbn}`);
    } catch (error) {
      console.error(`Error retrieving ISBN for ${title} by ${author}: ${error}`);
    }
  }
})();
